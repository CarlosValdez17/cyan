...
public class Registros {
    ...
    public Registros(Connection c) {
        cn = c;
    }

    public void tablaPendientes(DefaultTableModel modelo) {
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.fecha,v.total,v.usuario FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) where estadoServicio='PENDIENTE'";
            System.out.println("PENDIENTES: "+sql);
    ...
    public void tablaOrdenes(DefaultTableModel modelo, String estadoVenta, String estadoServicio) {
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.fecha,v.total,v.usuario "
                    + "FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) "
                    + "where (v.estado = '" + estadoVenta + "') " + estadoServicio;
    ...
    public void tablaReportes(DefaultTableModel modelo, String where) {
        ...
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.total,v.fecha,v.usuario,v.tipoPago FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) " + where;
            System.out.println(sql);
            CallableStatement cmd = cn.prepareCall(sql);
            ResultSet rs = cmd.executeQuery();
            while (rs.next()) {
    ...
    public void tablaFacturas(DefaultTableModel modelo) {
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.fecha,v.total FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) where factura = 'SI'";
            CallableStatement cmd = cn.prepareCall(sql);
            ResultSet rs = cmd.executeQuery();
            while (rs.next()) {
                Object[] datos = new Object[10];
    ...
    public void tablaFacturasFIN(DefaultTableModel modelo) {
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.fecha,v.total FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) where factura = 'FIN'";
            CallableStatement cmd = cn.prepareCall(sql);
            ResultSet rs = cmd.executeQuery();
            while (rs.next()) {
                Object[] datos = new Object[10];
    ...
    public void tablaTransferencias(DefaultTableModel modelo, String estado) {
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.fecha,v.total FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) where v.tipoPago = 'TRANSFERENCIA' AND v.estadoPago='" + estado + "'";
            //System.out.println(sql);
            CallableStatement cmd = cn.prepareCall(sql);
            ResultSet rs = cmd.executeQuery();
            while (rs.next()) {
    ...
    public void busquedaOrdenes(DefaultTableModel modelo, String tipo, String palabra) {
        try {
            String sql = "SELECT v.id,c.Nombre,v.concepto,v.fecha,v.total,v.usuario FROM ventas v INNER JOIN clientes c on(v.cliente=c.idCliente) where " + tipo + " like '" + palabra + "%'";
            //System.out.println(sql);
            CallableStatement cmd = cn.prepareCall(sql);
            ResultSet rs = cmd.executeQuery();
            while (rs.next()) {
