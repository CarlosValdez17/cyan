...
// *********************************METODOS PARA INICIAR SESION EN EL SISTEMA, LOGEO, CONTRSEÑAS ETC.. *******************************
    ...
    public String saludo(String usuario) {
        try {
            ...
            String sql = "SELECT nombre, apellido,icono,tipoUsuario FROM usuarios WHERE contrasena='" + usuario + "' ";
            CallableStatement cmd = cn.prepareCall(sql);
            ResultSet rs = cmd.executeQuery();
            ...
            while (rs.next()) {
                ...
                String a = datos[0] + " " + datos[1] + "," + datos[2] + "," + datos[3];
    ...
    public void añadirUsu(String nom, String ape, String contra, String tipo, String ico, String usu) {
        try {
            String sql = "insert into usuarios values(null,'" + nom + "','" + ape + "','" + contra + "','" + tipo + "', '" + ico + "','" + usu + "')";
            PreparedStatement cmd = cn.prepareCall(sql);
            cmd.execute();
            cmd.close();
        } catch (Exception ex) {
